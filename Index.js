...
if (processedSigs[sig]) return;
processedSigs[sig] = true;
...
let lastSig = allSigs[walletAddress] || null;
setInterval(async () => {
if (!isRunning) return;
try {
const sigs = await connection.getSignaturesForAddress(pubkey, {limit: 5});
if (!sigs.length) return;
const newSigs = sigs.filter(s => s.signature !== lastSig && !processedSigs[s.signature]);
if (!newSigs.length) return;
lastSig = newSigs[0].signature;
allSigs[walletAddress] = lastSig;
fs.writeFileSync(SIG_FILE, JSON.stringify(allSigs));
